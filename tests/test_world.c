...
Test(world, The_default_world)
{
	...
	cr_expect(w->shapes);
	cr_expect(w->shapes->next);
	cr_expect(!w->shapes->next->next);
}
...
Test(world, Precomputing_the_state_of_an_intersection)
{
	...
	s->sphere.valid = 1;
	t_intersection i = cons_intersection(4, s);

	t_comps comps = cons_comps(i, r);

	cr_expect(comps.t == i.t);
	cr_expect(comps.shape == i.object);
...
Test(world, The_hit_when_an_intersection_occurs_on_the_inside)
{
	...
	t_shape *s = cons_shape();
	s->sphere.valid = 1;
	t_intersection i = cons_intersection(1, s);
